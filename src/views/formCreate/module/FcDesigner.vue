  ...
  <ElContainer class="_fc-designer"
    ...
    <ElMain>
      <ElContainer style="height: 100%;">
        <el-aside class="_fc-l"
          ...
          <template v-for="(item, index) in menuList">
            <div class="_fc-l-group"
              ...
              <draggable :group="{name:'default', pull:'clone',put:false}"
                         :sort="false"
                         :list="item.list">
        ...
        <ElDialog :visible.sync="preview.state"
                  width="800px"
                  append-to-body>
          <FormCreate :rule="preview.rule"
                      :option="preview.option"
                      v-if="preview.state"></FormCreate>
        </ElDialog>
      </ElContainer>
    </ElMain>
  </ElContainer>
...
export default {
  ...
  methods: {
    ...
    validateChange(formData) {
      ...
      if (
        !this.activeRule ||
        this.validateForm.api.activeRule !== this.activeRule
      )
        return;
      if (!formData.type) {
            ...
            on: {
              ...
              copy: ({ self }) => {
                ...
                top.root.children.splice(
                  top.root.children.indexOf(top.parent) + 1,
                  0,
                  formCreate.copyRule(top.parent)
                );
              },
              active: ({ self }) => {
